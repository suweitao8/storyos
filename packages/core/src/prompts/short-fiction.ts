...
export function buildShortFictionDraftReviewUserPrompt(
  ...
  if (language === "en") {
    return [
      ...
      "The first line of your response MUST be exactly `REVIEW_DECISION: KEEP` when there is no blocking issue, or `REVIEW_DECISION: REVISE` when at least one concrete repair is required. Do not choose REVISE for optional polish.",
      "## Required Review Format",
  ...
  return [
    ...
    "第一行必须严格写 `REVIEW_DECISION: KEEP` 或 `REVIEW_DECISION: REVISE`：没有会阻止读者继续读的硬伤就写 KEEP，至少有一个具体、必须修复的问题才写 REVISE；不要因为可选润色而选择 REVISE。",
    "## 审稿输出格式（必须遵守）",
    "### 读者拉力与可保留内容",
    "### 必须修复的问题",
