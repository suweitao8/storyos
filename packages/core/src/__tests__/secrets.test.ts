...
describe("secrets", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "storyos-secrets-"));
  });
  ...
  describe("loadSecrets", () => {
    it("returns empty when .storyos/secrets.json does not exist", async () => {
  ...
  describe("saveSecrets", () => {
    it("creates .storyos dir and writes secrets file", async () => {
      await saveSecrets(root, {
        services: { deepseek: { apiKey: "sk-deep" } },
      });
      const raw = await readFile(join(root, ".storyos", "secrets.json"), "utf-8");
