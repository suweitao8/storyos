...
describe("listModelsForService (B8)", () => {
  const originalEnv = process.env.STORYOS_LLM_MODEL;
  const originalFetch = global.fetch;

  beforeEach(() => {
    delete process.env.STORYOS_LLM_MODEL;
  });

  afterEach(() => {
    if (originalEnv) process.env.STORYOS_LLM_MODEL = originalEnv;
    else delete process.env.STORYOS_LLM_MODEL;
  ...
  it("R4：env 补丁已删除 — STORYOS_LLM_MODEL 不再污染跨 service 菜单", async () => {
    process.env.STORYOS_LLM_MODEL = "my-secret-model";
    const models = await listModelsForService("anthropic");
