...
describe("config migration", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "storyos-migrate-"));
  });
  ...
  it("migrates old llm.provider+model+apiKey to services[] + secrets", async () => {
    ...
    await writeFile(join(root, "storyos.json"), JSON.stringify(oldConfig));
  ...
  it("does nothing if already in new format", async () => {
    ...
    await writeFile(join(root, "storyos.json"), JSON.stringify(newConfig));
