...
describe("ArchitectAgent — Phase 5 prose output", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "storyos-phase5-arch-"));
  });
  ...
  it("repairs missing architect sections once before failing the book creation flow", async () => {
    ...
    expect(repairMessages[0]?.content).toContain("你负责修复 architect 的输出格式");
    expect(repairMessages[0]?.content).toContain("不要重新发明一本书");
    expect(repairMessages[1]?.content).toContain("缺失 section：roles, book_rules");
...
describe("writeFoundationFiles — rhythm file is skipped when rhythmPrinciples is empty", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "storyos-phase5-rhythm-skip-"));
  });
