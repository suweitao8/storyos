...
instruction 必须自包含：题材方向、标题/暂定名、主角压力、核心冲突、情绪回报、封面视觉方向或目标短篇路径。生成完整短篇时同时填 shortRun：direction、chapters、charsPerChapter、cover；cover 默认 false，只在用户明确要求真实封面时再填 true；charsPerChapter 只能是每章 900-1200 字，不是整篇总字数。
...
instruction must be self-contained: genre direction, title/working title, protagonist pressure, core conflict, emotional payoff, cover direction, or target short path. For full short production, also fill shortRun: direction, chapters, charsPerChapter, cover; set cover to false unless the user explicitly asks for a real cover image; charsPerChapter is per-chapter 900-1200 Chinese chars, not total story length.
