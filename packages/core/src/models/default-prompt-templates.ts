...
export const DEFAULT_IMAGE_CHARACTER_TEMPLATE = `你的任务是根据用户提供的角色档案信息，为 GPT-Image-2 图片生成模型编写一段直接可用的中文角色参考图提示词。

## 画面布局（关键，必须严格遵守）
输出一张 16:9 横构图的角色参考图，整张图竖向均分为四列，从左到右依次是：

1. **胸部以上正面特写**：从头顶到胸口的半身正面像，占第一列。
   目的是清晰展示角色的面部细节、发型、上半身服装和胸前可见的身体标记。
   这个区域是为了后续分镜的中景、近景镜头提供参考，必须细节充分。
2. **正视图（全身）**：角色全身正面站立，面朝镜头，双臂自然下垂
3. **侧视图（全身）**：角色全身侧面站立（90度正侧面），面朝右侧
4. **背视图（全身）**：角色全身背面站立，背朝镜头，展示角色后脑发型、背部服装和背后可见的身体标记

四列的角色比例、身材、穿着、配饰、发型必须完全一致。

## 输出结构
先写画面布局指令，然后按以下顺序描述角色的视觉特征：
画面布局 → 年龄体型 → 面部特征 → 发型发色 → 服装（分件描述）→ 配饰 → 身体标记 → 辨识标志。

## 各字段规范
1. **画面布局**：按上面的布局指令开头，明确告诉模型这是一张"16:9角色参考图，竖向均分为四列：胸部以上正面特写、正视图全身、侧视图全身、背视图全身"
2. **角色年龄体型**：如"二十五岁偏瘦青年"、"三十岁左右中等身材女性"，要写出身高感觉和身材比例
3. **面部特征**：东亚人面孔（深棕色瞳孔、黑发），加上角色特有的面部特征（黑眼圈、疤痕、泪痣、酒窝、法令纹等）
4. **发型发色**：明确发型和颜色，如"凌乱黑色短发"、"齐肩栗色卷发"、"高马尾，碎刘海"
5. **服装（核心，逐件描述）**：把角色的每件衣物单独写清楚，每件都包含款式+主色+辅色+材质：
   - **上衣/外套**：如"深蓝色圆领卫衣，棉质，袖口和下摆有灰色罗纹收口"、"黑色修身西装外套，内搭白色衬衫"
...
## 强制规则
- 画面布局指令必须出现在提示词开头，明确描述四等分结构：胸像正面特写、正视图全身、侧视图全身、背视图全身
- 四个角度的穿着、比例、配饰、发型必须完全一致，确保角色在不同角度下的视觉统一性
...
输出：16:9横构图角色参考图，画面竖向均分为四列，从左到右依次是：胸部以上正面特写（从头顶到胸口，展示面部和上半身细节）、正视图全身（面朝镜头，双臂自然下垂）、侧视图全身（90度正侧面，面朝右侧）、背视图全身（背朝镜头，展示后脑发型和背部）。四个角度为同一角色，穿着比例完全一致。二十五岁偏瘦青年，身高约175cm，体型清瘦。东亚人面孔，深棕色瞳孔，黑发，眼下有淡淡黑眼圈，左眼角有一颗泪痣。凌乱黑色短发，刘海随意分开，后脑头发自然收拢。深蓝色圆领棉质卫衣，袖口和下摆有灰色罗纹收口，背面平整无图案。深灰色棉帆布工装裤，膝盖处有立体口袋，腰间系棕色牛皮皮带。白色低帮帆布鞋，橡胶底。左手腕戴黑色电子手表。左前臂内侧有一条约15cm的龙形黑色纹身，从手腕延伸到肘部。神情疲惫，正视图和背视图中双手插于裤袋，侧视图双臂自然下垂。`;
