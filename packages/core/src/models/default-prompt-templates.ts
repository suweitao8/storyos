...
## 画面布局（关键，必须严格遵守）
输出一张 16:9 横构图的角色参考图，整张图分为左右两个区域：

**左 1/3 区域——胸部以上正面特写**
从头顶到胸口的半身正面像，占画面左侧三分之一。
目的是清晰展示角色的面部细节、发型、上半身服装和胸前可见的身体标记。
这个区域是为了后续分镜的中景、近景镜头提供参考，必须细节充分。

**右 2/3 区域——三视图全身**
画面右侧三分之二，竖向均分为三列，从左到右依次是：
1. **正视图**：角色全身正面站立，面朝镜头，双臂自然下垂
2. **侧视图**：角色全身侧面站立（90度正侧面），面朝右侧
3. **透视角度图**：角色全身四分之三侧面（约45度），面朝镜头左前方
三个角度的角色比例、身材、穿着、配饰、发型必须完全一致。
...
## 各字段规范
1. **画面布局**：按上面的布局指令开头，明确告诉模型这是一张"16:9角色参考图，左侧三分之一为胸部以上正面特写，右侧三分之二为正视图、侧视图、透视角度图三列全身"
...
## 强制规则
- 画面布局指令必须出现在提示词开头，明确描述左1/3胸像+右2/3三视图的结构
- 三视图的三个角度穿着、比例、配饰、发型必须完全一致
...
输出：16:9横构图角色参考图，画面分为左右两部分。左侧三分之一为角色胸部以上正面特写，从头顶到胸口，清晰展示面部和上半身细节。右侧三分之二竖向均分为三列：正视图、侧视图（面朝右）、透视角度图（四分之三侧面），三个角度为同一角色的全身像，穿着比例完全一致。二十五岁偏瘦青年，身高约175cm，体型清瘦。东亚人面孔，深棕色瞳孔，黑发，眼下有淡淡黑眼圈，左眼角有一颗泪痣。凌乱黑色短发，刘海随意分开。深蓝色圆领棉质卫衣，袖口和下摆有灰色罗纹收口。深灰色棉帆布工装裤，膝盖处有立体口袋，腰间系棕色牛皮皮带。白色低帮帆布鞋，橡胶底。左手腕戴黑色电子手表。左前臂内侧有一条约15cm的龙形黑色纹身，从手腕延伸到肘部。神情疲惫，正视图和透视图中双手插于裤袋，侧视图双臂自然下垂。`;
