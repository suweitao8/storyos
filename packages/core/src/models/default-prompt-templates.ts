...
export const DEFAULT_IMAGE_SHOT_TEMPLATE = `你的任务是根据用户提供的分镜信息（画面描述、景别、旁白），为 GPT-Image-2 图片生成模型编写一段直接可用的中文分镜图片提示词。

## 核心原则
分镜图片是影视镜头的单帧画面——它必须是一个可拍摄的、有故事感的构图。与角色设定图不同，分镜图要展现的是"这一刻发生了什么"：人物的动作与情绪、场景的环境与氛围、光线的方向与色调，共同构成有叙事张力的画面。画幅为 16:9 横构图。

## 输出格式
输出一段流畅的中文描述段落，按以下顺序涵盖镜头画面的完整视觉要素：

1. **主体描述**：画面中的人物或焦点对象。写清外貌（穿着、发型、体态）、当前的动作（站立、奔跑、低头、转身）、面部表情或肢体语言（疲惫、警觉、恐惧、微笑）。如画面中有多个角色，按视觉主次逐一描述。
2. **场景环境**：主体所处的空间。写清地点（室内/室外、走廊、街道、车辆内部）、环境中的关键物件与背景层次（墙壁、窗户、家具、植被、建筑轮廓）。不要只写"室内"，要写出具体的空间质感。
3. **光线与色调**：光源方向（顶光、侧光、逆光、自然光）、明暗分布（高对比、均匀柔和）、色温与色调（暖黄、冷蓝、低饱和、高饱和）。光线要服务于画面情绪。
4. **构图与景别**：主体在画面中的位置（居中、偏左、占满画面）、景别（远景交代环境、全景人物全身、中景腰部以上、近景胸部以上、特写面部/手部）、拍摄角度（平视、俯拍、仰拍、倾斜）。
5. **氛围与情绪**：画面要传达的情绪基调（压抑、紧张、温暖、孤寂、悬疑、宁静）。用具体的视觉元素来烘托，而不是直接写"紧张氛围"。

## 补充规则
- 旁白信息用于理解镜头的情绪和叙事意图，不要把旁白文字写进画面
- 如果分镜标注了引用的角色/场景/道具（如【角色名】），在主体描述中使用对应的外貌特征和环境设定
- 景别（远景/中景/特写等）必须反映在构图描述中
- 动作和表情要具体：不要写"她很紧张"，写"她双手攥紧衣角，嘴唇微抿，眼神回避镜头"
- 光线描述要具体：不要写"昏暗的光线"，写"头顶一盏白炽灯发出惨白的光，在她脸上投下深重的阴影"

## 强制规则
- 提示词长度至少 50 个字
- 每个视觉要素只描述一次，不重复
- 画面要有叙事感——它是一个正在发生的瞬间，不是静态的摆拍
- 不要写文字、字幕、旁白内容出现在画面里
- 不要写摄影机型号、镜头参数等拍摄技术术语
- 直接输出提示词段落，不写解释、前缀或标题`;
...
export const DEFAULT_IMAGE_TEMPLATES: Record<ImagePromptKind, string> = {
  ...
  shot: DEFAULT_IMAGE_SHOT_TEMPLATE,
};
...
export const DEFAULT_IMAGE_STYLE_REALISTIC: Record<ImagePromptKind, string> = {
  ...
  shot: `写实电影摄影风格，画面有电影级质感和叙事张力。人物表情和动作真实自然，光影层次丰富有戏剧性，景深控制突出视觉焦点，色调和氛围服务于故事情绪。`,
};
...
export const DEFAULT_IMAGE_STYLE_CG3D: Record<ImagePromptKind, string> = {
  ...
  shot: `3D国漫风格，高质量CG影视画面渲染，人物建模精细，场景层次丰富。色彩饱满有影视感，光影对比有戏剧张力，构图和景别讲究，画面有强烈的叙事氛围和视觉冲击力。`,
};
