...
export function formatSqliteMemorySupportWarning(options?: {
  ...
  return `Node ${nodeVersion} does not support SQLite memory index; memory.db live sync will fall back to Markdown. Use Node 22+ or run 'storyos doctor'.`;
}
...
export async function inspectNodeRuntimePinFiles(root: string): Promise<NodeRuntimePinStatus> {
  ...
  return {
    ok: false,
    detail: `Missing or outdated: ${missing.join(", ")}. Run 'storyos doctor --repair-node-runtime'.`,
    missing,
  };
}
