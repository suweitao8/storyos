...
const STORYOS_CONFIG = JSON.stringify({
  name: "test-project",
  version: "0.1.0",
  language: "zh",
...
describe.skip("interactive-film-authoring confirm flow (stubbed LLM)", () => {
  let root: string;
  const prev = process.env.STORYOS_AGENT_LLM_STUB;
  beforeAll(() => { process.env.STORYOS_AGENT_LLM_STUB = "1"; });
  afterAll(() => {
    if (prev === undefined) delete process.env.STORYOS_AGENT_LLM_STUB;
    else process.env.STORYOS_AGENT_LLM_STUB = prev;
  });
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "if-confirm-"));
    await writeFile(join(root, "storyos.json"), STORYOS_CONFIG, "utf-8");
