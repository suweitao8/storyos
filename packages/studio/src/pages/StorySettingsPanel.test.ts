...
describe("groupStorySection", () => {
  it("classifies generated short-outline sections into useful display groups", () => {
    ...
    expect(groupStorySection({ file: "outline/v001.md#section-2", title: "人物与关系" })).toBe("settings");
  ...
  it("routes unmatched sections into settings", () => {
    expect(groupStorySection({ file: "misc.md", title: "杂项" })).toBe("settings");
  });

  it("builds navigation tabs without counts", () => {
    const groups = [
