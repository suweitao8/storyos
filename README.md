...
<p align="center">
  <strong>StoryOS 网页版上线！</strong>
  <a href="https://huohuaapi.com/apps">立刻体验</a>
</p>

---

StoryOS 是一个面向故事创作的 AI Agent 系统：长篇连载、独立短篇、剧本剧作、同人番外、仿写续写和开放世界互动，都可以从同一个工作台开始。支持 Studio、TUI、CLI 交互形式，把创意、设定、角色、记忆、审稿、修订、封面和互动状态交给智能体统一管理，让故事能持续生产、持续修改、持续玩下去。
...
StoryOS 1.6.2 把 Studio Chat 的协作能力补齐到更接近真实创作工作台：你可以上传文档、Markdown 和图片，让模型在回答和创作中直接读取；也可以中断长任务，把外部材料归档成可检索参考，并在 Studio 里直接调整关键提示词包。
...
StoryOS 1.6.0 把开放世界继续推进到互动影游、剧本和分镜工作台，同时引入可插拔 Skill 系统：专业能力可以被 Chat 自动调用，也可以由用户强制指定。写作、互动、研究和导出继续共享同一套 action surface，重动作确认后再执行，产物可以在 Studio 内查看和导出。
...
<p align="center">
  <img src="assets/interactive-film-e2e.png" width="900" alt="StoryOS 互动影游剧情树实测截图">
</p>

## v1.5.0 - StoryOS Play 发布，开放世界，用想象力游玩

StoryOS Play 发布和 Studio 体验升级：你可以用一句自然语言创建开放世界，让角色、物品、证据、关系和时间一起推进；也可以继续写长篇、做短篇、生成封面、改设定和查状态。系统会记住世界发生了什么，并在需要时把该看的上下文带给模型。

- **StoryOS Play**：新增开放世界与分支互动。支持自由动作、可点击选择、世界契约、非固定时间推进、角色 agent、物品 / 证据 / 关系状态、HUD 和自动配图。
...
- **模型与错误边界**：弱模型格式不稳时更少直接崩溃；模型服务错误、StoryOS 执行错误和图片生成错误会更清楚地区分，方便判断是配置、供应商还是系统问题。

<p align="center">
  <img src="assets/studio-play-1-5.png" width="900" alt="StoryOS Play Studio 开放世界界面">
</p>

**长篇小说** — 从创作简报建书，生成世界观、角色、卷纲、章节意图，按“写作 → 审稿 → 必要修订 → 状态结算”推进。上下文按 protected / compressible 分层组织，避免长书越写越乱。

**StoryOS Short** — Studio Chat 和 CLI 可以直接产出独立短篇：完整正文、大纲记录、审稿记录、简介卖点、封面提示词，并在配置封面服务后生成封面图。

**StoryOS Play** — 新增开放世界与分支互动。你可以用自然语言指定世界契约、时间推进方式、角色 agent、物品 / 证据 / 关系规则和视觉风格；系统维护世界状态、可点击选择、自由动作、HUD 和自动配图。
...
StoryOS 已发布为 [OpenClaw](https://clawhub.ai/narcooo/inkos) Skill，可被任何兼容 Agent（Claude Code、OpenClaw 等）直接调用：

```bash
clawhub install inkos          # 从 ClawHub 安装 StoryOS Skill
```

通过 npm 安装或克隆本项目时，`skills/SKILL.md` 已包含在内，🦞 可直接读取——无需额外从 ClawHub 安装。

安装后，Claw 应优先通过共享交互入口调用 StoryOS：

```bash
storyos interact --json --message "继续当前书，但把节奏再收紧一点"
```
...
这里的 skill 指 StoryOS Chat/Play/长篇写作内部可使用的专业能力包，和上面的 ClawHub Skill 不是同一个概念。它不会给模型额外执行权限，只提供专业规则、上下文需求和 prompt pack；创建、写入、编辑、生成图片仍然走 Studio 的工具权限和确认闸门。

可用方式：

- 在项目目录放置 `.storyos/skills/<skill-id>/SKILL.md`，Studio Chat 会在运行时自动加载。
- 或设置 `STORYOS_SKILL_DIRS=/abs/path/to/skills`，可指向单个 skill 目录，也可指向包含多个 skill 子目录的目录。多个目录按系统分隔符分隔。
...
当前 StoryOS 将 LLM 配置分成两条清晰路径：**Studio 用可视化服务配置**，**CLI / daemon / 部署环境支持 env 覆盖**。两者不会互相污染。
...
```text
provider bank 默认值
→ storyos.json 里的 services / 当前 service / defaultModel
→ .storyos/secrets.json 里的 service API Key
```

即使检测到 `~/.storyos/.env` 或项目 `.env`，Studio 也只会展示提示，不会用 env 覆盖 service、model、baseUrl 或 API Key。API Key 存在项目内的 `.storyos/secrets.json`，不会写进 `storyos.json`。
...
也可以手动写 `~/.storyos/.env` 或项目 `.env`：

```bash
STORYOS_LLM_PROVIDER=custom
STORYOS_LLM_BASE_URL=https://api.moonshot.cn/v1
STORYOS_LLM_API_KEY=sk-...
STORYOS_LLM_MODEL=kimi-k2.5

# 可选
STORYOS_LLM_SERVICE=moonshot                         # 推荐写；不写时会尽量从 baseUrl 自动识别
STORYOS_LLM_TEMPERATURE=0.7
STORYOS_LLM_THINKING_BUDGET=0
STORYOS_DEFAULT_LANGUAGE=zh
STORYOS_LLM_EXTRA_top_p=0.9
```
...
→ .storyos/secrets.json service key
→ global ~/.storyos/.env
→ project .env
→ 当前进程环境变量
→ CLI 参数
...
也就是说，CLI 默认可以复用 Studio 配好的服务和密钥；如果 env 里声明了 `STORYOS_LLM_SERVICE`、`STORYOS_LLM_MODEL`、`STORYOS_LLM_BASE_URL` 或 `STORYOS_LLM_API_KEY`，则作为覆盖层生效。旧 env 只写 `baseUrl + model + apiKey` 也能继续用，StoryOS 会尽量从 baseUrl 反推 service。

一次性指定服务或模型：

```bash
storyos write next --service google --model gemini-2.5-flash
storyos write next --service moonshot --model kimi-k2.5 --no-stream
storyos agent "继续写下一章" --api-key-env MOONSHOT_API_KEY
storyos doctor --service minimaxCodingPlan --model MiniMax-M2.7
```
...
storyos config show-models        # 查看当前路由
```
...
如果服务测试失败，优先检查服务商、模型和协议是否匹配。Google Gemini 的 AI Studio API Key 可用于 Gemini OpenAI-compatible endpoint；StoryOS 会自动禁用 Google 不支持的 OpenAI `store` 参数。MiniMax 默认走官方 OpenAI-compatible `/v1/chat/completions`，并优先使用可工作的非流式 transport，避免流式返回 usage 但无正文的问题；`MiniMax-M3*` 会默认关闭 thinking 返回，M2.x thinking 由上游限制无法关闭。

### LLM 配置更新

- **Studio / CLI 配置隔离**：Studio 固定使用服务页配置和 `.storyos/secrets.json`；CLI、daemon、部署环境支持 env 覆盖和一次性命令参数。
...
- **Google Gemini 兼容修复**：AI Studio API Key 可直接用于 Gemini OpenAI-compatible endpoint，StoryOS 会自动禁用 Google 不支持的 OpenAI `store` 参数。
...
- **旧 env 兼容**：老的 `STORYOS_LLM_BASE_URL + STORYOS_LLM_MODEL + STORYOS_LLM_API_KEY` 仍可用于 CLI；没有 `STORYOS_LLM_SERVICE` 时会尝试从 baseUrl 反推服务商。
...
- **TUI 仪表盘**：`storyos tui` 进入终端全屏交互，适合键盘流用户。
- **外部 Agent 入口**：`storyos interact --json --message "..."` 仍是 OpenClaw / 其他 agent 的结构化入口。
- **原子命令保留**：`plan` / `compose` / `draft` / `audit` / `revise` / `write next` 仍适合脚本和高级用户。

### 写第一本书

```bash
storyos book create --title "吞天魔帝" --genre xuanhuan  # 创建新书
storyos write next 吞天魔帝      # 写下一章（草稿 → 审计 → 按配置修订）
storyos status                   # 查看状态
storyos review list 吞天魔帝     # 审阅草稿
storyos review approve-all 吞天魔帝  # 批量通过
storyos export 吞天魔帝          # 导出全书
storyos export 吞天魔帝 --format epub  # 导出 EPUB（手机/Kindle 阅读）
```
...
`storyos style analyze` 分析参考文本，提取统计指纹（句长分布、词频特征、节奏模式）和 LLM 风格指南。`storyos style import` 将指纹注入指定书籍，后续所有章节自动采用该风格，修订者也会用风格标准做审计。

### 创作简报

`storyos book create --brief my-ideas.md` 传入你的脑洞、世界观设定、人设文档。建筑师 agent 会基于简报生成故事设定（`story_bible.md`）和创作规则（`book_rules.md`），而非凭空创作；同时把简报落盘到 `story/author_intent.md`，让这本书的长期创作意图不会只在建书时生效一次。
...
```bash
storyos plan chapter 吞天魔帝 --context "本章先把注意力拉回师徒矛盾"
storyos compose chapter 吞天魔帝
```
...
- 如果正文超出允许区间，StoryOS 最多只会追加 1 次纠偏归一化（压缩或补足），不会直接硬截断正文
- 如果 1 次纠偏后仍然超出 hard range，章节照常保存，但会在结果和 chapter index 里留下长度 warning / telemetry

### 续写已有作品

`storyos import chapters` 从已有小说文本导入章节，自动重建结构化状态、章节摘要、伏笔、角色关系和可读 Markdown 投影，支持 `第X章` 和自定义分割模式、断点续导。导入后 `storyos write next` 可继续创作。

### 同人创作

`storyos fanfic init --from source.txt --mode canon` 从原作素材创建同人书。支持四种模式：canon（正典延续）、au（架空世界）、ooc（性格重塑）、cp（CP 向）。内置正典导入器、同人专属审计维度和信息边界管控——确保设定不矛盾。

### 多模型路由

不同 Agent 可以走不同模型和 Provider。写手用 Claude（创意强），审计用 GPT-4o（便宜快速），雷达用本地模型（零成本）。`storyos config set-model` 按 agent 粒度配置，未配置的自动回退全局模型。

### 守护进程 + 通知推送

`storyos up` 启动后台循环自动写章。管线会自动推进可处理的非关键问题；需要人工判断的问题会暂停并留下可审结果。通知推送支持 Telegram、飞书、企业微信、Webhook（HMAC-SHA256 签名 + 事件过滤）。日志写入 `storyos.log`（JSON Lines），`-q` 静默模式。

### 本地模型兼容

支持任何 OpenAI 兼容接口（Studio 里新增自定义服务，或 CLI 使用 `--provider custom` / `STORYOS_LLM_PROVIDER=custom`）。服务测试会尝试不同协议和流式开关组合，并保存或提示可用 transport。Fallback 解析器处理小模型不规范输出，流中断时自动恢复部分内容。

### 可靠性保障

每章自动创建状态快照，`storyos write rewrite` 可回滚任意章节。写手动笔前输出自检表（上下文、资源、伏笔、风险），写完输出结算表，审计员交叉验证。文件锁防止并发写入。写后验证器含跨章重复检测和十余条硬规则自动 spot-fix。
...
模型输出上限由 provider bank 的模型卡管理；`llm.extra` / `STORYOS_LLM_EXTRA_*` 中的保留键（max_tokens、temperature、model、messages、stream 等）会被自动过滤，防止意外覆盖核心请求参数。

---

## 工作原理

StoryOS 现在有两条主要运行线：长篇 / 短篇生产线负责生成可交付文本；Play 运行线负责持续互动世界。它们共享模型配置、Studio Chat、确认动作和产物预览，但状态结构不同。

<p align="center">
  <img src="assets/arch-system.svg" width="900" alt="StoryOS 整体系统架构">
</p>

长篇每一章默认按“规划 → 编排 → 写作 → 审计 → 必要修订 → 状态同步”运行：

<p align="center">
  <img src="assets/arch-pipeline.svg" width="900" alt="StoryOS 章节生产管线">
</p>
...
如果审计不通过，默认管线只做一次"修订 → 再审计"；仍未解决的问题会保留在结果和状态里，交给人工或后续命令继续处理。需要更强自动闭环时，可以运行 `storyos config set writing.reviewRetries 3` 把修订轮数调高。
...
<p align="center">
  <img src="assets/arch-memory.svg" width="900" alt="StoryOS 长期记忆与状态">
</p>

### 控制面与运行时产物

除了运行时状态，StoryOS 还把“护栏”和“自定义”拆成可审阅的控制层：
...
StoryOS 提供四种交互方式，底层共享同一组原子操作：

### 1. 完整管线（一键式）

```bash
storyos write next 吞天魔帝          # 写草稿 → 审计 → 按配置自动修订
storyos write next 吞天魔帝 --count 5 # 连续写 5 章
```

`write next` 现在默认走 `plan -> compose -> write` 的输入治理链路，审计后的自动修订轮数默认是 1。若你需要回退到旧的 prompt 拼装路径，可在 `storyos.json` 中显式设置：
...
```bash
storyos plan chapter 吞天魔帝 --context "本章重点写师徒矛盾" --json
storyos compose chapter 吞天魔帝 --json
storyos draft 吞天魔帝 --context "本章重点写师徒矛盾" --json
storyos audit 吞天魔帝 31 --json
storyos revise 吞天魔帝 31 --json
```
...
```bash
storyos agent "帮我写一本都市修仙，主角是个程序员"
storyos agent "写下一章，重点写师徒矛盾"
storyos agent "先扫描市场趋势，然后根据结果创建一本新书"
```
...
<p align="center">
  <img src="assets/studio-dashboard.png" width="760" alt="StoryOS Studio 开始创作入口">
</p>

<p align="center">
  <strong>StoryOS Short 手机封面</strong><br>
  <img src="assets/inkos-short-demo-cover.png" width="260" alt="短篇封面">
</p>

<p align="center">
  <strong>StoryOS Play 恋爱互动</strong><br>
  <img src="assets/play-openworld-romance.png" width="560" alt="恋爱互动">
</p>

<p align="center">
  <strong>StoryOS Play 侦探互动</strong><br>
  <img src="assets/play-openworld-detective.png" width="560" alt="侦探互动">
</p>

<p align="center">
  <strong>StoryOS Play 物品配图</strong><br>
  <img src="assets/play-item-warcraft.png" width="560" alt="物品配图">
</p>

第一张是当前 Studio 的本地实测截图。后面四张来自 StoryOS Short 和 StoryOS Play 的真实本地生成结果：短篇封面用于手机端缩略图点击，Play 图用于展示开放世界、侦探证据、互动场景和物品视觉能力。
...
| 命令                                          | 说明                                                                                         |
| ------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `storyos init [name]`                         | 初始化项目（省略 name 在当前目录初始化）                                                                    |
| `storyos book create`                         | 创建新书（`--genre`、`--platform`、`--chapter-words`、`--target-chapters`、`--brief <file>` 传入创作简报） |
| `storyos book update [id]`                    | 修改书设置（`--chapter-words`、`--target-chapters`、`--status`）                                    |
| `storyos book list`                           | 列出所有书籍                                                                                     |
| `storyos book delete <id>`                    | 删除书籍及全部数据（`--force` 跳过确认）                                                                  |
| `storyos genre list/show/copy/create`         | 查看、复制、创建题材                                                                                 |
| `storyos plan chapter [id]`                   | 生成下一章的 `intent.md`（`--context` / `--context-file` 传入当前指令）                                  |
| `storyos compose chapter [id]`                | 生成下一章的 `context.json`、`rule-stack.yaml`、`trace.json`                                       |
| `storyos write next [id]`                     | 完整管线写下一章（`--words` 覆盖字数，`--count` 连写，`-q` 静默模式）                                            |
| `storyos write rewrite [id] <n>`              | 重写第 N 章（恢复状态快照，`--force` 跳过确认，`--words` 覆盖字数）                                              |
| `storyos draft [id]`                          | 只写草稿（`--words` 覆盖字数，`-q` 静默模式）                                                             |
| `storyos audit [id] [n]`                      | 审计指定章节                                                                                     |
| `storyos revise [id] [n]`                     | 修订指定章节                                                                                     |
| `storyos agent <instruction>`                 | 自然语言 Agent 模式                                                                              |
| `storyos review list [id]`                    | 审阅草稿                                                                                       |
| `storyos review approve-all [id]`             | 批量通过                                                                                       |
| `storyos status [id]`                         | 项目状态                                                                                       |
| `storyos export [id]`                         | 导出书籍（`--format txt/md/epub`、`--output <path>`、`--approved-only`）                           |
| `storyos radar scan`                          | 扫描平台趋势                                                                                     |
| `storyos fanfic init`                         | 从原作素材创建同人书（`--from`、`--mode canon/au/ooc/cp`）                                              |
| `storyos short run`                           | 生成独立短篇包（正文、简介卖点、封面提示词、可选封面图）                                                               |
| `storyos eval [id]`                           | 生成质量评估报告（支持 `--json`、章节范围）                                                                 |
| `storyos consolidate [id]`                    | 归并长篇章节摘要，降低长书上下文压力                                                                         |
| `storyos interact`                            | 外部 agent / CLI 自然语言入口（`--json`、`--message`、`--book`）                                       |
| `storyos config set-global`                   | 设置 CLI / daemon / 部署环境的全局 LLM env（`~/.storyos/.env`）                                         |
| `storyos config show-global`                  | 查看全局配置                                                                                     |
| `storyos config set/show`                     | 查看/更新项目配置                                                                                  |
| `storyos config set-model <agent> <model>`    | 为指定 agent 设置模型覆盖（`--base-url`、`--provider`、`--api-key-env` 支持多 Provider 路由）                |
| `storyos config remove-model <agent>`         | 移除 agent 模型覆盖（回退到默认）                                                                       |
| `storyos config show-models`                  | 查看当前模型路由                                                                                   |
| `storyos doctor`                              | 诊断配置问题（显示 effective config mode、来源、API 连通性和提供商兼容性提示）                                       |
| `storyos detect [id] [n]`                     | AIGC 检测（`--all` 全部章节，`--stats` 统计）                                                         |
| `storyos style analyze <file>`                | 分析参考文本提取文风指纹                                                                               |
| `storyos style import <file> [id]`            | 导入文风指纹到指定书                                                                                 |
| `storyos import canon [id] --from <parent>`   | 导入正传正典到番外书                                                                                 |
| `storyos import chapters [id] --from <path>`  | 导入已有章节续写（`--split`、`--resume-from`）                                                        |
| `storyos analytics [id]` / `storyos stats [id]` | 书籍数据分析（审计通过率、高频问题、章节排名、token 用量）                                                           |
| `storyos update`                              | 更新到最新版本                                                                                    |
| `storyos studio` / `storyos`                    | 启动 Web 工作台（`-p` 指定端口，默认 4567；Studio 使用服务页配置，不使用 env 覆盖）                                    |
| `storyos tui`                                 | 启动终端全屏 TUI                                                                                 |
| `storyos up / down`                           | 启动/停止守护进程（`-q` 静默模式，自动写入 `storyos.log`）                                                      |
...
- ~~`packages/studio` Web UI 工作台（Vite + React + Hono）~~ — 已发布，`storyos` 或 `storyos studio` 启动
...
StoryOS 的 agent 运行时构建在 [pi](https://github.com/badlogic/pi-mono)（`@mariozechner/pi-ai` 与 `@mariozechner/pi-agent-core`，作者 Mario Zechner）之上。感谢 pi 提供的扎实底座。
